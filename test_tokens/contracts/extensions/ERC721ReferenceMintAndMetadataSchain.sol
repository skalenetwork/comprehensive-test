...
contract ERC721ReferenceMintAndMetadataSchain is MessageSender {
    ...
    function sendTokenToMainnet(address receiver, uint256 tokenId) external {
        require(
            ERC721(erc721ContractOnSchain).getApproved(tokenId) == address(this),
            "Not allowed ERC721 Token"
        );
        ERC721(erc721ContractOnSchain).transferFrom(msg.sender, address(this), tokenId);
        string memory tokenURI = ERC721(erc721ContractOnSchain).tokenURI(tokenId);
        ERC721(erc721ContractOnSchain).burn(tokenId);
